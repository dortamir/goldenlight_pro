...
## Security Grant Hardening Patch (migration 025)

Closes the column-grant gap flagged at the end of Stage 9 - `authenticated` (shared by customer and admin sessions) had SELECT access to more columns than the app actually reads, on `purchase_reports`, `receipt_ocr_results`, `receipt_ocr_lines`, `receipt_line_matches`, and `receipt_manual_items`. Row-level RLS was already correct everywhere (verified again here) and is **not** touched by this migration - only column-level grants change.

### A real bug found and fixed while writing this migration

The first draft used plain `revoke select (col, col) on table t from authenticated` for every table. Live testing (simulating a real customer session, not just reading grant metadata) proved this was a **no-op** for three of the five tables - `purchase_reports`, `receipt_ocr_lines`, and `receipt_manual_items` were each created with a *whole-table* `grant select ... to authenticated` (no column list). In PostgreSQL, a column-level `REVOKE` and a table-level `GRANT` are separate, additive ACL entries - revoking a column does nothing while the broader table-level grant still exists. This is also, incidentally, the exact reason `022_ocr_normalization.sql`'s own column-level revoke of `receipt_ocr_lines`'s Stage 2 columns never actually took effect - discovered live during an earlier stage of this session and, at the time, wrongly attributed to "something re-granted it." It was never revoked to begin with, for this reason. `022` is not edited (it's already applied) - `receipt_ocr_lines` is now correctly and fully closed via `025` instead, using `revoke select on table t from authenticated` (removing the whole-table grant) followed by an explicit `grant select (safe columns) to authenticated`.

### A second real bug found and fixed: a cross-table RLS dependency

Fully revoking `receipt_ocr_lines` broke the ADMIN'S own (unmodified) `receipt_line_matches` query with a genuine `permission denied for table receipt_ocr_lines` error - caught by testing the admin's exact query sequence end-to-end, not just checking grants in isolation. Root cause: `receipt_line_matches`'s own customer-ownership RLS policy (`007_create_product_matching.sql`, untouched by this migration) joins *through* `receipt_ocr_lines` to check report ownership. RLS policy expressions evaluate with the *querying* role's own privileges, not an elevated one - so once `authenticated` had zero access to `receipt_ocr_lines`, that policy became impossible to even evaluate, for anyone, including admin. Fixed by keeping exactly the two non-sensitive structural columns that policy's join actually touches (`id`, `ocr_result_id` - both plain UUIDs, no content) granted, while every real content column (`raw_text`, `raw_item`, every `*_confidence` column, every Stage 2 `normalized_*`/`normalization_*` column) stays fully revoked.

### Follow-up (same migration, still unapplied): full closure of receipt_line_matches/receipt_ocr_results/receipt_ocr_lines

The first version of this migration left `receipt_ocr_results.raw_text` and `receipt_line_matches`'s match-detail columns grant-accessible, because `adminReportService.js`'s `getAdminReportDetail()` read both via a **plain** `.from(...).select(...)` query, and admin/customer share the same `authenticated` role - documented at the time as a known, honest limitation rather than left silently unresolved. That gap is now closed, entirely by moving those two admin reads behind `SECURITY DEFINER` RPCs, the same pattern `get_admin_ocr_lines()`/`get_admin_manual_items()` already used:

- `receipt_ocr_results`: `adminReportService.js` now calls `get_admin_ocr_result(p_report_id)` - an `is_admin()`-gated RPC that has existed since `021_ocr_azure_document_intelligence.sql` but was never actually called by any screen until now. No new database function was needed.
- `receipt_line_matches`: a new RPC, `get_admin_receipt_line_matches(p_report_id)`, replaces the old plain select *and* the separate `products` sku/name lookup that used to follow it - the RPC now does both the report-scoped match lookup and the product join server-side in one call, saving a round trip.

With both admin reads moved off plain table queries, `authenticated` needs **zero** direct SELECT on `receipt_ocr_results` and `receipt_line_matches` - both are now a plain `revoke select on table ... from authenticated` with no re-grant at all. This, in turn, also removed the *only* reason `receipt_ocr_lines` still kept `id`/`ocr_result_id` grant-readable (the cross-table RLS policy on `receipt_line_matches` that joined through it, described above) - since `receipt_line_matches` itself is now fully closed to direct `authenticated` access, that join is never evaluated for a plain session any more, so `receipt_ocr_lines` was narrowed one step further, to zero grant as well.

### What could and couldn't be fully closed

**All five tables are now fully closed to direct customer/admin-shared `authenticated` access beyond exactly the safe columns customer screens use** (`purchase_reports`, `receipt_manual_items`) - `receipt_ocr_results`, `receipt_ocr_lines`, and `receipt_line_matches` all have **zero** direct SELECT grant. Every admin read of OCR/matching data now goes through an `is_admin()`-gated `SECURITY DEFINER` RPC (`get_admin_ocr_result`, `get_admin_ocr_lines`, `get_admin_receipt_line_matches`, `get_admin_manual_items`), none of which are reachable by a normal customer - calling any of them as a non-admin returns the same `not_admin` (`42501`) rejection every other admin-gated RPC in this schema already gives, verified live.

### Verification

Every claim above was verified live via isolated `BEGIN...ROLLBACK` transactions simulating real customer and real admin sessions (never a permanent write) - not by reasoning about grants in the abstract. Confirmed: the customer's exact columns (`getMyPurchaseReports`/`getPurchaseReportById`/`getReceiptManualItems`) all still work; `reviewed_by`/`admin_note` are genuinely rejected (`42501 permission denied`); direct `SELECT` on `receipt_ocr_results`, `receipt_ocr_lines`, and `receipt_line_matches` is genuinely rejected for a normal customer session; a normal customer calling `get_admin_receipt_line_matches()` directly is genuinely rejected; cross-user row access is still denied (RLS unaffected); and the admin's full `getAdminReportDetail()` sequence (`purchase_reports` plain read → `get_admin_ocr_result()` RPC → `get_admin_ocr_lines()` RPC → `get_admin_receipt_line_matches()` RPC → `get_admin_manual_items()` RPC) succeeds end-to-end with real data - including the real matched SKU (`6003028`), the real unresolved/unmatched rows, and a real saved manual item with quantity 42 / unit price 195. No customer query needed to change; two admin queries in `adminReportService.js` were changed from plain table selects to RPC calls (see above).

### Deployment

Not yet applied to the live database - validated only via rolled-back transactions, matching the pattern used for every migration in this session.
...
- SECURITY GRANT HARDENING PATCH (migration 025, not yet applied live): the customer-column over-exposure flagged in Stage 9's SECURITY FINDING above is now addressed by a real migration - see "Security Grant Hardening Patch (migration 025)" above for the exact grants, the two ACL/RLS bugs found and fixed while writing it, and the follow-up (same migration) that fully closes receipt_ocr_results/receipt_ocr_lines/receipt_line_matches to zero direct customer access by moving adminReportService.js's two remaining plain OCR/match queries behind is_admin()-gated SECURITY DEFINER RPCs (get_admin_ocr_result, and a new get_admin_receipt_line_matches).
